...
**Python** · [JavaScript](https://github.com/PRGuard-dev/demo-javascript) · [TypeScript](https://github.com/PRGuard-dev/demo-typescript) · [Java](https://github.com/PRGuard-dev/demo-java) · [C#](https://github.com/PRGuard-dev/demo-csharp) · [Go](https://github.com/PRGuard-dev/demo-go) · [Rust](https://github.com/PRGuard-dev/demo-rust) · [C++](https://github.com/PRGuard-dev/demo-cpp) · [PHP](https://github.com/PRGuard-dev/demo-php) · [Ruby](https://github.com/PRGuard-dev/demo-ruby)
...
3. GitHub redirects you to your copy.
   - The title at the top-left now reads **your-username/demo-python**.
...
3. On GitHub's install screen, choose **Only select repositories**, pick
   your **demo-python** fork, and click <kbd>Install</kbd>.
...
2. A banner lists each connected demo fork's context file — click
   <kbd>Import →</kbd> next to the **demo-python** fork you just connected.
...
1. In **your fork**, open [`app.py`](app.py) — clicking that link inside your
...
> (**your-username/demo-python**) before clicking the green button. On some
...
> [!TIP]
> **Deep Context.** The context file's `@ref:src/db.py` line pulls a compact
...
Paste this at the end of [`app.py`](app.py):
...
def format_receipt(order):
    ...
    return f"Order {order['id']}: ${order['total_cents'] / 100}"
```
...
| ------- | :---------: | ------- |
| 🔑 A live payment key hardcoded in `app.py` | **Human** | Rotate the key with the payment provider, then load it from an environment variable. A committed key is a compromised key — moving it is not enough, and no AI can un-leak a secret. |
| 💸 Currency built by hand in `app.py` (`total_cents / 100`) | **AI** | *Fix with AI* swaps the hand-rolled string for the shared `money()` helper in `src/db.py` — the same helper Deep Context placed in front of the auditor. Float division drops trailing zeros (`1200` → `$12.0`); `money()` formats cents exactly. |
